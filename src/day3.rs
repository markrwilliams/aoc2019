...
impl Move {
    ...
    fn go(&self, panel: &mut Panel, wire_id: i8, steps: &mut i64) {
        ...
        let (
            ...
            Direction::Up => {
                (
                    Box::new(cycle_x.iter().cycle().map(|x| *x)),
            ...
            Direction::Left => {
                (
                    ...
                    Box::new(cycle_y.iter().cycle().map(|y| *y)),
                )
            },
            Direction::Right => {
                (
        ...
        for (x, y) in xs.zip(ys) {
            ...
            panel.oy = y;
            *steps += 1;
            let cell = &mut panel.grid.entry((panel.ox, panel.oy)).or_insert_with(|| HashMap::new());
            cell.entry(wire_id).or_insert(*steps);
            match cell.iter().find(|(other_wire_id, _)| **other_wire_id != wire_id) {
                Some((_, other_wire_steps)) => {
                    panel.overlaps.insert((panel.ox, panel.oy), *steps + *other_wire_steps);
                },
                None => (),
            };
        }
...
impl Panel {
    ...
    fn png(&self, path: &str) {
        let (
            ...
            i64::max_value(), i64::max_value(), i64::min_value(), i64::min_value(),
        );
        ...
        for (x, y, pixel) in img.enumerate_pixels_mut() {
            ...
            *pixel = image::Rgb(
                match self.grid.get(&(translated_x, translated_y)) {
                    Some(_) => [255, 255, 255],
                    None => [0, 0, 0],
                });
        }
...
mod tests {
    ...
    fn basic_parse() {
        assert_eq!(
            ...
            vec![Wire{path: vec![Move{direction: Direction::Right, distance: 75}]}],
        )
    }
...
U62,R66,U55,R34,D71,R55,D58,R83"
            ...
            vec![
                Wire{path: vec![Move { direction: Right, distance: 75 },
                                Move { direction: Down, distance: 30 },
                                Move { direction: Right, distance: 83 },
                                Move { direction: Up, distance: 83 },
                                Move { direction: Left, distance: 12 },
                                Move { direction: Down, distance: 49 },
                                Move { direction: Right, distance: 71 },
                                Move { direction: Up, distance: 7 },
                                Move { direction: Left, distance: 72 }]},
                Wire{path: vec![Move { direction: Up, distance: 62 },
                                Move { direction: Right, distance: 66 },
                                Move { direction: Up, distance: 55 },
                                Move { direction: Right, distance: 34 },
                                Move { direction: Down, distance: 71 },
                                Move { direction: Right, distance: 55 },
                                Move { direction: Down, distance: 58 },
                                Move { direction: Right, distance: 83 }] }],
        )
    }
